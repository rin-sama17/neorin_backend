...
class CalculationFactory
{
    ...
    public function make(string $strategyKey): CalculationStrategy
    {
        $class = self::STRATEGIES[$strategyKey] ?? null;

        if (!$class) {
            throw new InvalidArgumentException(
                "Calculation strategy [{$strategyKey}] is not registered."
            );
        }
    ...
    public function calculateForItem(CartItem $item, CustomProductItem $productItem): array
    {
        ...
        $strategy = $this->make($profile->strategy_type);
